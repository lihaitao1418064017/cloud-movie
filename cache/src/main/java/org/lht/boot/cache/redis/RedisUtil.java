...
public class RedisUtil<K, V> {
     ...
     * @return 对应的多个键值
     */
    public Map<Object, V> hmGet(String key) {
     ...
     * @return true成功 false失败
     */
    public boolean hmSet(String key, Map<String, V> map, long time) {
        try {
            redisTemplate.opsForHash().putAll(key, map);
            if (time > 0) {
                expire(key, time);
     ...
     * @return 成功个数
     */
    public long sSet(String key, V... values) {
        try {
     ...
     * @return
     */
    public boolean lSet(String key, V value, long time) {
        try {
            redisTemplate.opsForList().rightPush(key, value);
            if (time > 0) {
                expire(key, time);
     ...
     * @return
     */
    public boolean lSet(String key, List<V> value) {
        try {
     ...
     * @return
     */
    public boolean lSet(String key, List<V> value, long time) {
        try {
            redisTemplate.opsForList().rightPushAll(key, value);
            if (time > 0) {
                expire(key, time);
     ...
     * @param message 消息内容
     */
    public void convertAndSend(String channel, V message) {
